...
def mash_i_vs_asig_to_create_filtered_new_asigdatacsv_3(genome_i):
    "GENOME -> ASIG_DATA | compare genome to asig_data"

    output_i = genome_i.replace('.fna', '')
    input_i = output_i + ".msh"
    with open('asig_data_3.csv', 'w') as outfile:
        subprocess.call(['mash', "dist", input_i, "asig_3.msh", "-p", "24"], stdout=outfile)

    asig_data = pd.read_csv("asig_data_3.csv", sep='\t', header=None)
    ...
    return asig_4, clique_prev_list, value_prev_clique_T2, clique_prev, tabla_asig

def list_of_connections_i_vs_possible_pseudoclique_3(asig_dist_3, distance_input, clique_prev_list, tabla_asig):

    lowest_value_genome_3 = asig_dist_3.loc[asig_dist_3['value'] == min(asig_dist_3['value'])].values[0, 1]
    lowest_value_clique_3 = tabla_asig.loc[tabla_asig['Genome'] == lowest_value_genome_3].values[0, 3]
    clique_3 = tabla_asig.loc[(tabla_asig['T2'] == lowest_value_clique_3)]
    clique_list_3 = clique_3['Genome'].tolist()
    ...
    # distance dataframe of clique T1 elements
    # asig_1 = asig_dist_3[asig_dist_3['Target'].isin(clique_prev_list)]
    # asig_3 = asig_1[asig_1['Source'].isin(clique_prev_list)]
    # asig_4 = asig_3.drop_duplicates()

    satelite_posibles = asig_dist_3[asig_dist_3['value'] < distance_input].dropna()
    satelite_posibles = satelite_posibles[satelite_posibles['Target'].isin(clique_list_3)]
    satelite_posibles = satelite_posibles.drop_duplicates()
    satelite_list = satelite_posibles['Target'].tolist()

    return  satelite_list, clique_list_3, lowest_value_clique_3

def mash_paste_i_asig_3(genome_i):
    "GENOME -> ASIG.MSH | Create file  "
    output_i = genome_i.replace('.fna', '')
    input_i = output_i + ".msh"
    subprocess.call(['mash', "paste", "tmp4", input_i, "asig_3.msh"])
    subprocess.call(['mv', 'tmp4.msh', "asig_3.msh"])

def add_satellite_to_tabla_asig_3(genome_i, lowest_value_clique_input, tabla_asig_input):

    tabla_asig_input.loc[tabla_asig_input.Genome == genome_i, 'T3'] = lowest_value_clique_input
...
def from_clique_dist_to_largest_clique_3(asig_dist_matrix_3,clique_prev_list, distance3, clique_prev):


    noasig_3 = clique_prev[clique_prev['T3'].isnull()]
    clique_prev_list_3 = noasig_3['Genome'].tolist()


    #distance dataframe of clique T1 elements and T3 nan
    asig_1 = asig_dist_matrix_3[asig_dist_matrix_3['Target'].isin(clique_prev_list_3)]
    asig_2 = asig_1[asig_1['Source'].isin(clique_prev_list_3)]
    asig_4 = asig_2.drop_duplicates()
...
def add_first_clique_to_asig_data_and_asigmsh_3(tabla_asig_input, largest_clique_input, clique_prev):
    "CLIQUE -> ASIG_DATA & ASIG.MSH | the first clique asign to asig_data and create of asig.msh"

    tabla_asig_max_value = (clique_prev.T3.max())
    ...
    for genome_asig in largest_clique_input:
        tabla_asig_input.loc[tabla_asig_input.Genome == genome_asig, 'T3'] = tabla_asig_max_value_new
...
def append_satellites_to_new_clique_tabla_asig_3(satellites, tabla_asig_input, tabla_asig_max_value_new):

    for satellite_asig in satellites:
        tabla_asig_input.loc[tabla_asig_input.Genome == satellite_asig, 'T3'] = tabla_asig_max_value_new
