...
def from_clique_dist_to_largest_clique_2(genome_i, asig_dist_matrix_2, distance2):
    ...
    asig_1 = asig_dist_matrix_2[asig_dist_matrix_2['Target'].isin(clique_prev_list)]
...
def add_first_clique_to_asig_data_and_asigmsh_2(largest_clique_input):
    "CLIQUE -> ASIG_DATA & ASIG.MSH | the first clique asign to asig_data and create of asig.msh"

    tabla_asig = pd.read_csv("tabla_asig.csv")
    #
    # for genome in largest_clique_input:
    #     tabla_asig.loc[tabla_asig.Genome == genome, 'T2'] = 1

    count1 = 0

    for genome_asig in largest_clique_input:
        tabla_asig.loc[tabla_asig.Genome == genome_asig, 'T2'] = 1
...
def append_satellites_to_new_clique_tabla_asig_2(satellites, tabla_asig_input):

    for satellite_asig in satellites:
        tabla_asig_input.loc[tabla_asig_input.Genome == satellite_asig, 'T2'] = 1
